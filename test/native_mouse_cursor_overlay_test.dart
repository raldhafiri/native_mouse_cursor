...
void main() {
  testWidgets('renders its child (pass-through when not forced)',
    ...
    await tester.pumpWidget(_wrap(
      const NativeMouseCursorOverlay(child: Text('hello')),
    ));
  ...
  testWidgets('force overlay mounts and rebuilds without error',
      (tester) async {
    await tester.pumpWidget(_wrap(
      const NativeMouseCursorOverlay(force: true, child: SizedBox()),
    ));
    await tester.pump();
    // Toggling force back off should not throw.
    await tester.pumpWidget(_wrap(
      const NativeMouseCursorOverlay(force: false, child: SizedBox()),
    ));
    await tester.pump();
